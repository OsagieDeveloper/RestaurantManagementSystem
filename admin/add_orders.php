...
if(isset($_POST['submit'])){
    ...
    // Fetch user_id from reservation or session if available
    $user_id = isset($reservation['user_id']) ? $reservation['user_id']  : 0;

    // Insert into orders table
    $stmt = $mysqli->prepare("INSERT INTO orders (user_id, reservation_id, total, status, created_at) VALUES (?, ?, ?, ?, NOW())");
    $stmt->bind_param("isds", $user_id, $reservation_id, $total, $status);
        ...
        <main>
            ...
            <div class="reservation-details">
                ...
                <div class="detail-item">
                    ...
                    <span><?php echo isset($reservation['reservation_id']) ? $reservation['reservation_id'] : 'N/A'; ?></span>
                </div>
                
                <div class="detail-item">
                    <span class="detail-label">Table number:</span>
                    <span><?php echo isset($reservation['table_number']) ? $reservation['table_number'] : 'N/A'; ?></span>
                </div>
                
                <div class="detail-item">
                    <span class="detail-label">Number of guest:</span>
                    <span><?php echo isset($reservation['num_guests']) ? $reservation['num_guests'] : 'N/A'; ?></span>
                </div>
                
                <div class="detail-item">
                    <span class="detail-label">Date & Time:</span>
                    <span><?php echo isset($reservation['date_time']) ? $reservation['date_time'] : 'N/A'; ?></span>
                </div>
                
                <div class="detail-item">
                    <span class="detail-label">Status:</span>
                    <span><?php echo isset($reservation['status']) ? $reservation['status'] : 'N/A'; ?></span>
                </div>
            </div>
            ...
            <span class="order-id">Order ID: <?php echo isset($reservation['reservation_id']) ? $reservation['reservation_id'] : 'New Order'; ?></span>
            <form method="POST">
                    ...
                    <div class="food-selection">
                        ...
                        <div class="food-category">
                            ...
                            <select name="menu_item_id_n">
                                ...
                                <?php
                                $stmt = $mysqli->prepare("SELECT * FROM menus WHERE type = 'main_dishes'");
                                $stmt->execute();
                                $result = $stmt->get_result();
                                while ($row = $result->fetch_assoc()) {
                                    echo "<option value=" . $row['id'] . ">" . $row['name'] . " - $" . $row['price'] . "</option>";
                        ...
                        <div class="food-category">
                            ...
                            <select name="menu_item_id_d">
                                ...
                                <?php
                                $stmt = $mysqli->prepare("SELECT * FROM menus WHERE type = 'desserts'");
                                $stmt->execute();
                                $result = $stmt->get_result();
                                while ($row = $result->fetch_assoc()) {
                                    echo "<option value=" . $row['id'] . ">" . $row['name'] . " - $" . $row['price'] . "</option>";
